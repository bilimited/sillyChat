...
abstract class STLorebookImporter {
  ...
  static LorebookModel? fromJson(Map<String, dynamic> json,
    ...
    } else if (json['entries'] is Map<String, dynamic>) {
      entries = (json['entries'] as Map<String, dynamic>).values.toList();
    } else {
      entries = [];
    }
    ...
    entries.forEach((entry) {
      ...
      LorebookItemModel item = LorebookItemModel(
        ...
        isActive: entry['enabled'] ?? !entry['disable'],
        ...
        keywords:
            ((entry['keys'] ?? entry['key'] ?? []) as List<dynamic>).join(','),
        activationType: type,
      );
