...
class _EditCharacterPageState extends State<EditCharacterPage>
  ...
  Future<void> _pickImage(bool isAvatar) async {
    ...
    final path = await ImageUtils.selectAndCropImage(context,
        isCrop: isAvatar,
        fileName: '${isAvatar ? "avatar" : "bg"}_${widget.characterId}_$t');
    ...
    return ListView(
      ...
      children: [
        ...
        ExpandableTextField(
          ...
          decoration:
              const InputDecoration(labelText: '简略介绍', helperText: "角色的简短描述"),
          minLines: 2,
          maxLines: 3,
        ),
        const SizedBox(height: 16),
        ExpandableTextField(
          ...
          decoration:
              const InputDecoration(labelText: '开场白', helperText: "角色的第一条对话"),
          extraActions: [
            if (_character != null)
              buildIconTextButton(
                context,
                ...
                onPressed: () {
                  customNavigate(MoreFirstMessagePage(character: _character!),
                      context: context);
                },
              )
          ],
        ...
        ExpandableTextField(
          ...
          decoration: const InputDecoration(
              labelText: '角色设定', helperText: '角色的背景、性格、能力等核心设定'),
          maxLines: 15,
        ),
      ],
    );
          ...
          children: [
            if (_character?.lorebookIds.isEmpty ?? true)
              const Padding(
                  ...
                  child: Text('未绑定任何世界书', style: TextStyle(color: Colors.grey)))
            else
              ...(_character!.lorebookIds.map((id) {
                ...
                return ListTile(
                  ...
                  trailing: IconButton(
                      ...
                      onPressed: () =>
                          setState(() => _character!.lorebookIds.remove(id))),
                  onTap: () => customNavigate(
                      LoreBookEditorPage(
                          lorebook: LoreBookController.of.getLorebookById(id)),
                      context: context),
                );
              })),
            Row(
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: _backgroundPath != null
            ? ClipRRect(
                borderRadius: BorderRadius.circular(12),
                child: Image.file(File(_backgroundPath!),
  ...
  void _onBindLorebook() {
    final availableBooks = _lorebookController.lorebooks
        .where((lb) =>
            lb.type == LorebookType.character &&
            !(_character?.lorebookIds.contains(lb.id) ?? false))
        .toList();
        ...
        child: availableBooks.isEmpty
            ...
            : ListView.builder(
                ...
                itemBuilder: (c, i) => ListTile(
                  ...
                  onTap: () {
                    setState(() =>
                        _character?.lorebookIds.add(availableBooks[i].id));
                    Get.back();
                  },
                ),
              ),
  ...
  Widget _buildPlayerSetting() {
    return ListView(
      ...
      children: [
        ...
        TextFormField(
            controller: _nickNameController,
            decoration: const InputDecoration(labelText: '我的名字')),
        const SizedBox(height: 16),
        TextFormField(
            controller: _briefController,
